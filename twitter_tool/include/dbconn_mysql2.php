...
//--------------------------------------------------------------------------------
//�K��N���X
class BaseMySqlConnection2
{
	var $m_DBConnection2 = "0";
	var $m_rscount = 0;
	var $m_aryrs;
	var $m_connect = false;
	...
	function ConnectStart() {
		$this->m_DBConnection2 = mysql_connect(S_DB_HOST2, S_DB_USER2, S_DB_PSWD2);
		mysql_query("SET NAMES utf8", $this->m_DBConnection2);
		// �f�[�^�x�[�X�̑I��
		$this->m_selectdb = mysql_select_db(S_DB_NAME2, $this->m_DBConnection2);
		if(isset($this->m_DBConnection2)) {
			$this->m_connect = true;
		}

		return $this->m_DBConnection2;
	}

	function TransactionErrorCheck() {
		global $aryenvkey;
		if( mysql_error($this->m_DBConnection2) ) {
			$this->m_totalerror = mysql_error($this->m_DBConnection2)."\n";
			$bError = true;
		}
	...
	function ConnectClose() {
		if($this->m_connect) {
			...
			mysql_close($this->m_DBConnection2);
	...
	function execute($sQuery) {
		$this->m_aryrs[++$this->m_rscount] = new MySqlRecordControl2($this->m_DBConnection2,$sQuery);
	}
...
class MySqlRecordControl2
{
	...
	function __construct($DBConnection2,$wkQuery) {
		$this->m_query = mb_convert_encoding($wkQuery, DB_ENCODE, IF_ENCODE);
		$this->m_objrs = @mysql_query($this->m_query, $DBConnection2);
//		echo $wkQuery."<br>";
		if($this->m_objrs) {
			$this->next();
		} else {
			...
			$this->m_errmsg.= 'pg_last_error='.@mysql_error($DBConnection2)."\n";
		}
...
//�h���N���X
class DBConnection2 extends BaseMySqlConnection2
{
	function __construct($bManualCommit = false) {
